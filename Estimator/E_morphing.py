...
def generate_wtilde_layer(input_layer):
    wtilde_component_layers = [Lambda(lambda t: 1. + 0.*t[:, 0])
                                     (input_layer), \
                               Lambda(lambda t: t[:, 1])(input_layer),
                               Lambda(lambda t: t[:, 1]*t[:, 1])(input_layer),
                               Lambda(lambda t: t[:, 1]*t[:, 1]*t[:, 1])(input_layer),
                               Lambda(lambda t: t[:, 1]*t[:, 1]*t[:, 1]*t[:, 1]) \
                                     (input_layer),
                               Lambda(lambda t: t[:, 0]) \
                                     (input_layer),
                               Lambda(lambda t: t[:, 0]*t[:, 1]) \
                                     (input_layer),
                               Lambda(lambda t: t[:, 0]*t[:, 1]*t[:, 1]) \
                                     (input_layer),
                               Lambda(lambda t: t[:, 0]*t[:, 1]*t[:, 1]*t[:, 1]) \
                                     (input_layer),
                               Lambda(lambda t: t[:, 0]*t[:, 0]) \
                                     (input_layer),
                               Lambda(lambda t: t[:, 0]*t[:, 0]*t[:, 1]) \
                                     (input_layer),
                               Lambda(lambda t: t[:, 0]*t[:, 0]*t[:, 1]*t[:, 1]) \
                                     (input_layer),
                               Lambda(lambda t: t[:, 0]*t[:, 0]*t[:, 0]) \
                                     (input_layer),
                               Lambda(lambda t: t[:, 0]*t[:, 0]*t[:, 0]*t[:, 1]) \
                                     (input_layer),
                               Lambda(lambda t: t[:, 0]*t[:, 0]*t[:, 0]*t[:, 0]) \
                                     (input_layer)]
